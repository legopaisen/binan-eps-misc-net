...
namespace Modules.SearchAccount
{
    public partial class frmSearchARN : Form
    {
        ...
        private void bntList_Click(object sender, EventArgs e)
        {
            ...
            if (SearchCriteria == "QUE-NEW" || SearchCriteria == "QUE-REN" || SearchCriteria == "QUE" || SearchCriteria == "QUE-ELEC-OCC")
            {
                //strWhereCond = $" where application_que.main_application = 1 and ";
                if(SearchCriteria == "QUE-ELEC-OCC")
                    strWhereCond = $" where application_que.main_application = 0 and arn like 'AN%' and"; //AFM 20201204 REQUESTED BY BINAN AS PER MITCH - FIXED "AN" FOR NEW APPLICATION
                else
                ...
                if (SearchCriteria != "QUE" && !SearchCriteria.Contains("QUE-ELEC-OCC"))
