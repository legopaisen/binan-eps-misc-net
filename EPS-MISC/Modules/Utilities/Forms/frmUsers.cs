...
namespace Modules.Utilities.Forms
{
    public partial class frmUsers : Form
    {
        ...
        private void LoadUserLevel()
        {
            ...
            sQuery = "select * from user_level WHERE USER_LEVEL = 'TELLER' OR USER_LEVEL = 'ADMINISTRATOR' OR USER_LEVEL = 'SUPERUSER' ";
        ...
        private void UpdateModuleList()
        {
            ...
            if (sTemp == "ADMINISTRATOR" || sTemp == "ADMINISTRATOR|")
                sQuery = $"select right_desc,right_code from rights where right_code in (select module_code from user_level_rights where user_level = '{cmbUserLevel.Text.ToString()}') and (right_code like 'COL-%' OR right_code = 'SS' OR right_code = 'SSS' OR right_code = 'SUA' OR right_code = 'SBK' OR right_code = 'SA' OR right_code = 'RRO' OR right_code = 'SC' OR right_code = 'SUM') order by right_desc"; //add exclusive rights here
            else
                sQuery = $"select right_desc,right_code from rights where right_code in (select module_code from user_level_rights where user_level = '{cmbUserLevel.Text.ToString()}') and right_code <> 'BOA' and  (right_code like 'COL-%' OR right_code = 'SS' OR right_code = 'SSS' OR right_code = 'SUA' OR right_code = 'SBK' OR right_code = 'SA' OR right_code = 'RRO' OR right_code = 'SC' OR right_code = 'SUM') order by right_desc"; //add exclusive rights here
        ...
        private void SaveUser(string sTransCode)
        {
            ...
            sQuery = "delete from users_rights where user_code = '" + m_sUserCode + "' and right_code like 'COL-%' ";
            db.Database.ExecuteSqlCommand(sQuery);
