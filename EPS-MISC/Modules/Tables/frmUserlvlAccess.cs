...
namespace Modules.Tables
{
    public partial class frmUserlvlAccess : Form
    {
        ...
        private void frmUserlvlAccess_Load(object sender, EventArgs e)
        {
            ...
            result.Query = "select * from rights where (right_code like 'COL-%' OR RIGHT_CODE = 'SS' OR RIGHT_CODE = 'SSS' OR RIGHT_CODE = 'SUA' OR RIGHT_CODE = 'SBK' OR RIGHT_CODE = 'SA' OR RIGHT_CODE = 'RRO' OR RIGHT_CODE = 'SC' OR RIGHT_CODE = 'SUM') order by right_code, right_desc"; //add exclusive rights here
            if(result.Execute())
                while(result.Read())
        ...
        private void cboDesignation_SelectedIndexChanged(object sender, EventArgs e) //AFM 20200803
        {
            ...
            bool hasVal = false;
            result.Query = "select UR.*, RI.right_desc from user_level_rights UR, rights RI where UR.MODULE_CODE = RI.RIGHT_CODE and user_level = '" + cboDesignation.SelectedItem.ToString().Trim() + "' AND (RI.RIGHT_CODE LIKE 'COL-%' OR RI.RIGHT_CODE = 'SS' OR RI.RIGHT_CODE = 'SSS' OR RI.RIGHT_CODE = 'SUA' OR RI.RIGHT_CODE = 'SBK' OR RI.RIGHT_CODE = 'SA' OR RI.RIGHT_CODE = 'RRO' OR RI.RIGHT_CODE = 'SC' OR RI.RIGHT_CODE = 'SUM') order by UR.module_code, RI.right_desc"; //add exclusive rights here
            if (result.Execute())
                while (result.Read())
                {
                    hasVal = true;
        ...
        private void btnEdit_Click(object sender, EventArgs e) //AFM 20200803
        {
            ...
            else if(btnEdit.Text == "Save")
            {
                ...
                { 
                    ...
                    rSet2.Query = "DELETE FROM USER_LEVEL_RIGHTS WHERE USER_LEVEL = '" + userlvl + "' and (module_code like 'COL%' OR module_code = 'SS' OR module_code = 'SSS' OR module_code = 'SUA' OR module_code = 'SBK' OR module_code = 'SA' OR module_code = 'RRO' OR module_code = 'SC' OR module_code = 'SUM')"; //add exclusive rights here
